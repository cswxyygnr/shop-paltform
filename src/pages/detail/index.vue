            ...
            <div class="chooseArea">
              ...
              <dl v-for="(saleAttr, index) in spuSaleAttrList" :key="saleAttr.id">
                ...
                <dd changepirce="0" :class="{ active: saleAttrValue.isChecked == 1 }" v-for="(
                    saleAttrValue, index
                  ) in saleAttr.spuSaleAttrValueList" :key="saleAttrValue.id" @click="changeChecked(saleAttrValue, saleAttr.spuSaleAttrValueList)
                    ">
...
export default {
  ...
  methods: {
    ...
    async addOrUpdateCart() {
      ...
      let result =  await this.$store.dispatch('addOrUpdateCart', { skuId:this.$route.params.skuId, skuNum: this.skuNum })
